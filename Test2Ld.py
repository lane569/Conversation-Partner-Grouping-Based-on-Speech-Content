...
def TransK(k_result):
    ...
    for _ in result:
        _=[_.append('0.0') for __ in range(24)]
    for _ in k_result:
        for x in _:
            for y in _:
                result[x-1][y-1]='1'
...
def main(Max_score):
    try:
        ...
        kcluster=KMeansClusterer(n,distance=nltk.cluster.cosine_distance,repeats=25)
        assigned_clusters=kcluster.cluster(np.array(lda_similarity_matrix),True)
        ...
        #for  i in range(n):
            #print('%d: %s'%(i,count[i]))
        outcome=TransK(count)
        #保存並輸出結果
        save_similarity_matrix( outcome, OUTPUT_PATH )
